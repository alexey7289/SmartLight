...
* *master-функция* — центральный оркестратор. Вызывает все Check-функции и обновляет глобальное состояние UI.
	* *syncUI()* — центральная функция, которая запускает все Check-функции, собирает результаты валидации и обновляет глобальное состояние интерфейса (состояние кнопки сохранения).
